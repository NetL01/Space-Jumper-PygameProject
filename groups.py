...
class Monster(Sprite):
    def __init__(self, x_location, y_location, x_jumper, y_jumper, time_to):
        Sprite.__init__(self, x_location, y_location)
        self.init_image('img/monster.png')
        # задаём прямую, по которой монстр будет двигаться
        # к прошлому месту джампера
        self.Vx = (x_jumper - x_location) / time_to
        self.Vy = (y_jumper - y_location) / time_to
...
class Jumper(Sprite):
    ...
    # 1 - жив, 0 - no
    alive = 1
    # скорость джампера по оси y
    ySpeed = 5
    # начальные координаты спавна джампера, можно задавать в config.py
    x = jumper_start_position[0]
    y = jumper_start_position[1]

    def __init__(self, name):
        pygame.sprite.Sprite.__init__(self)
        self.name = name
        self.img_r = pygame.image.load('img/Jumper.png').convert()
